...
async def process_file(file_path: str, output_path: str, col_letter: str = "G") -> schemas.FileCreate:
    try:
        ...
        for row in sheet.iter_rows(min_row=7, max_row=37):
            ...
            output_sheet["{}{}".format(col_letter, index)] = floor(amount.seconds/3600)
            log.debug(
        ...
        output_sheet["{}38".format(col_letter)] = "=SUM({}7:{}37)".format(col_letter, col_letter)
        output_sheet["{}39".format(col_letter)] = "=SUM({}7:{}21)".format(col_letter, col_letter)
        output_sheet["{}40".format(col_letter)] = "=SUM({}22:{}37)".format(col_letter, col_letter)
...
def extract_data(event: FilesToProcessEvent, price_unit: int = 1, col_letter: str = "G", currency: str = "CAD") -> bool:
    event_handled = False
    try:
        for file in event.files_to_process:
            xlsx_file = Path(file)
            wb_obj = openpyxl.load_workbook(xlsx_file)
            sheet = wb_obj.active
